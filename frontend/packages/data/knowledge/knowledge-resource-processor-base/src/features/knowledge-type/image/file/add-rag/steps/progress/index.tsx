...
/**
 * Rag-mode progress step for image file upload.
 *
 * Differences from the legacy `<ImageProcess />`:
 ...
 *     same navigation primitive the legacy process footer's "Done" button
 *     uses, so behaviour stays consistent across modes.
 *   - The legacy ANNOTATION step is dropped (rag has no ExtractPhotoCaption
 *     wiring yet — see Task 0 of the rag-flow alignment plan). The store
 *     still carries `annotationType`; we snapshot it for the CreateDocument
 *     caption_type, defaulting to Auto when unset (matches store init).
 *
 * The store shape is the same `ImageFileAddStore` used by the legacy wizard
 * (the upload step is reused as-is, so we must read from the same store).
 * Doc IDs are NOT pulled from the store; they come from the `CreateDocument`
 * response captured locally below — the legacy wizard never persists doc IDs
 * to the store as a top-level array either, it persists them indirectly via
 * `progressList[].documentId`.
 */
export const ImageProgress: FC<ContentProps<ImageFileAddStore>> = props => {
